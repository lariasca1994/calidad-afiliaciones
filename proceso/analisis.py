...
CONSULTAS = {
    "tablero": """
        ...
        FROM consolidado
        UNION ALL
        SELECT 'Registros brutos', (SELECT COUNT(*) FROM vw_universo_tramites), NULL
        UNION ALL
        ...
        FROM consolidado
        UNION ALL
        SELECT 'Novedades laborales', (SELECT COUNT(*) FROM PROCESO_IRL), NULL
        UNION ALL
    ...
    "fuentes": """
        ...
        LEFT JOIN (SELECT canal_origen, COUNT(*) AS n
                   FROM consolidado GROUP BY canal_origen) c
...
def _tabla(cursor, titulo: str, consulta: str) -> None:
...
def mostrar_tablero(conexion) -> None:
    cursor = conexion.cursor()

    _tabla(cursor, "Cifras principales", CONSULTAS["tablero"])
    _tabla(cursor, "Clasificación del trámite", CONSULTAS["clasificacion"])
    _tabla(cursor, "Aporte y depuración por fuente", CONSULTAS["fuentes"])
    _tabla(cursor, "Calidad por fuente (%)", CONSULTAS["calidad"])
    _tabla(cursor, "Tipos de documento fuera de catálogo", CONSULTAS["sin_homologar"])

    cursor.close()


def exportar(conexion, carpeta: Path) -> None:
    """Exporta el consolidado a CSV, para Power Query o Excel."""
    import csv
